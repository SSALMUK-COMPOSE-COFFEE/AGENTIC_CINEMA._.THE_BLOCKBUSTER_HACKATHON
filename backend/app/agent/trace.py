...
def after_tool(tool: BaseTool, args: dict, tool_context: ToolContext, tool_response: Any) -> Any:
    ...
    if hydrator is not None:
        ...
        if parsed is not None:
            ...
            emit({"type": "tool_result", "agent": tool_context.agent_name, "name": tool.name, "result": truncate(hydrated)})
            return hydrated
    emit({"type": "tool_result", "agent": tool_context.agent_name, "name": tool.name, "result": truncate(tool_response)})
    return None
